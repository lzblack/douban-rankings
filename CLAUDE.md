...
`douban-rankings` 是纯数据 scraper：爬取多个外部电影/剧/书/音乐等榜单，建立 **"豆瓣 subject ID → 所在榜单与排名"** 的反向索引，以静态 JSON 发布在 `https://rank.douban.zhili.dev`（GitHub Pages + CNAME），供姐妹项目 `lzblack/userscripts/douban-rating-hub.user.js` 消费。
...
- 产出目录：`data/`
- GitHub Pages 发布源：`main` branch / `/data`（URL 干净，不含 `/data/` 前缀）
- CNAME 文件放 `/data/CNAME`，内容一行：`rank.douban.zhili.dev`
- 对外 URL：
