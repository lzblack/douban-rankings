...
`douban-rankings` 是纯数据 scraper：爬取多个外部电影/剧/音乐榜单，建立 **"豆瓣 subject ID → 所在榜单与排名"** 的反向索引，以静态 JSON 发布在 `https://rank.douban.zhili.dev`（Cloudflare Pages），供姐妹项目 `lzblack/userscripts/douban-rating-hub.user.js` 消费。
...
- ❌ 书 / 舞台剧 / 游戏 / 播客（**book 品类已评估后放弃**——一本书在豆瓣常有 10+ 版本 dbid 分散，单一映射无法覆盖，ROI 低；其他品类暂不考虑）
- ❌ 实时 API、自托管部署
...
`config/*-snapshot.json` 和 `config/manual-mapping.yaml` **gitignored，仅本地**——这些是维护者的 curation 成果 + 反爬侧信道，不进 public repo。生成或更新 snapshot 后无需 commit；直接：
```
pnpm run update          # 跑 pipeline，产出 data/*.json
git add data/            # 只 commit 公开的产出
git commit -m "data: refresh <source>"
git push
```

**CI 不能跑 pipeline**（Actions runner 没有 snapshot 文件）——`.github/workflows/update.yml` 的 schedule 应保持禁用，refresh 完全手动：本地跑 fetcher → 本地跑 pipeline → push `data/`。

Snapshot 文件不在或过期不阻塞其他 source：缺 snapshot 的 source 标 failed，overall=degraded，其余照跑（所以就算本地忘跑某个 fetcher 也不会全盘 stuck）。
