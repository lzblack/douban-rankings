...
## 维护任务：Criterion snapshot

`criterion.com` 边缘过滤云 IP（Actions runner 403；residential IP 200）。Pipeline 因此不直接 HTTP 拉 Criterion 列表，而是读取 `config/criterion-snapshot.json`——由维护者在家用命令生成并 commit：

```
pnpm run fetch:criterion-snapshot    # residential IP only
git add config/criterion-snapshot.json && git commit -m "data: refresh criterion snapshot" && git push
```

Cadence：季度一次足够（Criterion 每月新增 5-10 spine）。pipeline 读 snapshot 的方式在 `src/sources/criterion.mjs` 的 `scrape()` 里；snapshot 文件不在或过期不会阻塞 IMDb Top 250 的正常 run（Criterion 标 failed，overall=degraded，其他 source 照跑）。
...
- **PtGen archive**（<https://ourbits.github.io/PtGen/>）：约 40 万条 IMDB ↔ 豆瓣 subject id 映射，由 R酱 Rhilip 等 OurBits 社区维护。是 `src/matchers/imdb-to-douban.mjs` 第二层的数据源，显著降低对豆瓣 search endpoint 的打扰。
- **rank4douban**（<https://github.com/eddiehe99/rank4douban>）：matcher 三层策略（manual → community map → search）的思路来源。
- 感谢前人脚本 [豆瓣资源下载大师 (GreasyFork 329484)](https://greasyfork.org/scripts/329484) 的致谢实践，本项目沿用相同风格。
